...
/**
 * Convert keys + values, a full key-value mapping, or a handle to an OXL dict.
 *
 * This implements most of the flexible parameter conversion.
 *
 * @param a1 Input keys, key-value rows, or a string handle
 * @param a2 Input values or omitted
 */
auto to_oxl_dict(/*const*/ xloper12* a1, /*const*/ xloper12* a2)
{
...
/**
 * Compute a new date given a base date, tenor, adjustment rule, and calendar.
 */
OA_XLL_EXPORT() AxComputeDate(
  /*const*/ xloper12* a1,
  /*const*/ xloper12* a2) OA_ACCEL_SAFE()
{
  OA_ACCEL_SAFE_RETURN(oxl::OxlComputeDate(to_oxl_dict(a1, a2)));
}

OA_ACCEL_EXPORT_FUNC(AxComputeDate)
  .category("AXL Time")
  // TODO: have web documentation since the help is limited to 255 chars
  .help(
    "Compute a new date given a base date, tenor, adjustment rule, and "
      "calendar.\n"
    "\n"
    "The input has the following key-value format:\n"
    "\n"
  ...
  .arg("keys_or_dict", "Rows of input keys, key + value rows, or a handle")
  .arg("maybe_values", "Rows of input values (may be omitted)");

/**
 * Add a given number of business days to a starting date.
 */
OA_XLL_EXPORT() AxAddBusinessDays(
  /*const*/ xloper12* a1,
  /*const*/ xloper12* a2) OA_ACCEL_SAFE()
{
  OA_ACCEL_SAFE_RETURN(oxl::OxlAddBusinessDays(to_oxl_dict(a1, a2)));
}

OA_ACCEL_EXPORT_FUNC(AxAddBusinessDays)
  .category("AXL Time")
  .help(
    "Add the specified number of business days to the starting date.\n"
    "\n"
    "The input has the following key-value format:\n"
    "\n"
    "Base_Date: date, e.g. 2024-05-06\n"
