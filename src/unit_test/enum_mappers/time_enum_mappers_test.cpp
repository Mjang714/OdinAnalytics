...
// helper struct to represent an invalid argument for the enum name mappers. this
// is just a convenient way to represent the invalid cases in the test input tuple
template <typename T>
struct enum_name_mapper_invalid_argument {
    static constexpr auto input = "INVALID";
};

// helper to check if a type is an enum_name_mapper_invalid_argument. this is
// used to determine whether to EXPECT_THROW or EXPECT_EQ in the test
template <typename T>
...
// partial specialization of enum_name_mapper_test for enum_name_mapper_invalid_argument
// to allow us to call the test with the invalid input
template <typename T>
struct enum_name_mapper_test<enum_name_mapper_invalid_argument<T>>
  : enum_name_mapper_test<T> {};

// all the test cases where we map string -> enum for all the different enums
// in our library. we also include some invalid cases to test that the mappers
// throw the expected exceptions for invalid input
