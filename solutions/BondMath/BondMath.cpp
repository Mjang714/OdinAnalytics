...
{
    //Convert Upper case String to 
    std::string payment_freq = boost::to_upper_copy(payment_freq_input);
    try
    {
        if (payment_freq == "PA")
        {
        ...
        else if (payment_freq == "SA")
        {
            return SemiAnum;

        }
        else if (payment_freq == "Q")
        {
            return Quarterly;
        }
        else if (payment_freq == "M")
        {
            return Monthly;
        }
        else
...
std::vector<double> createTimeMaturitySchedule(int paymentFreq, double years) 
{
    ...
    int numberOfMaturities = static_cast<int> (paymentFreq*years);
...
//since we have an implmentation with Int no real need to redfine it imho. Convert compouding string to an int from enums
double getDuration(double years, double couponRate, double bondPrice, std::string compoundingStr)
{
    ...
    getDuration(years, couponRate, bondPrice, paymentFreqInt);
    return 0.0;
}
